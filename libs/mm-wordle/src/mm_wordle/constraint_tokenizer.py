"""Dense constraint-state tokenizer for Wordle (the V2/V3 representation).

Promoted from ``wordle2/tokenizer.py`` (ADR-5 in docs/v3-design.md) so the V2 and
V3 exercises share one source of truth for the representation. V2 keeps its local
copy untouched; V3 uses this one. Behavior is identical — ``test_constraint_tokenizer``
mirrors the V2 tokenizer spec.
...
Vocabulary (265 tokens):
  ...
  53-182:  yellow tokens (position-major): a-yellow-1..z-yellow-1, .., a-yellow-5..
...
class ConstraintTokenizer:
    ...
    def empty_prompt(self) -> list[int]:
        """The turn-1 / zero-constraint prompt: ``[bos] ? ? ? ? ? [sep]``.

        V3 pre-training conditions every word on this empty state (ADR-2).
        """
        return [self.bos_id, *([_UNKNOWN_ID] * 5), self.sep_id]
